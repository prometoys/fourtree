...
function bracketCount(field, answer_id)
{
    ...
    var oc = document.getElementsByName( 'opencount_' + answer_id )[0];
    if ( !oc ) return;

    var cc = document.getElementsByName( 'closedcount_' + answer_id )[0];
    if ( !oc ) return;

    var btn = document.getElementById( 'syntaxtree_draw_btn_' + answer_id );
    if ( !btn ) return;
